...
class FiltersViewController: UIViewController {
    ...
    @objc func clearAllFilters() {
        ...
        var query = String(format: Constants.defaultQueryProductList, selectedSort, "\(self.categoryID ?? ""):\(CurrentFacet.shared.queryFilterTmp)")
        
        if self.isSearch() {
            query = self.searchString
        }
        
        self.productViewModel.getProductListWithFilter(filter:query , nextPageEnabled: false) { isSuccess, _ in
            if isSuccess {
                self.updateFilterButton(filterValue: CurrentFacet.shared.filterValue)
            } else {
    ...
    @IBAction func filterButtonTapped(_ sender: Any) {
        ...
        if productViewModel.productTotalCount == 0 {
            clearAllFilters()
        }
        
        // ATInternet
        ATInternetTaggingManager.sharedManager.sendTrackClick(clickLibelle: TaggingData.kVoirResultats , pageName: nil, chapter1: TaggingData.kProductsList, chapter2: TaggingData.kFiltre, level2: TaggingData.kCommerceLevel)
    }
    
    func getProductListWithFilter() {
        var query = ""

        if self.isSearch() {
            query = searchString + "::\(CurrentFacet.shared.queryFilterTmp)"
        }
        else {
            query = String(format: Constants.defaultQueryProductList, selectedSort, "\(self.categoryID ?? ""):\(CurrentFacet.shared.queryFilterTmp)")
        }

...
- (NSString*)productAvailabilityWarningURL:(NSString*)productId {
    return [self.restPrefix stringByAppendingString:FORMAT(@"/products/%@/alerts/stock", productId)];
}

- (NSString*)userOrderInvoiceURLForUserId:(NSString*)userId andOrderCode:(NSString*)code {
    return [self.restPrefix stringByAppendingString:FORMAT(@"/users/%@/orders/%@/invoice", userId, code)];
}
...
- (NSString*)userCurrentCartURLForUserId:(NSString*)userId  {
    return [self.restPrefix stringByAppendingString:FORMAT(@"/users/%@/carts/current", userId)];
}
...
- (NSString*)renewOrderForUserId:(NSString*)userId code:(NSString *)code {
    return [self.restPrefix stringByAppendingString:FORMAT(@"/users/%@/orders/%@/renew", userId, code)];
}
...
//Payment Paypal
- (NSString*)userPaymentModesUrlForUserId:(NSString*)userId andCartId:(NSString*) cartId {
    return [self.restPrefix stringByAppendingString:FORMAT(@"/users/%@/carts/%@/paymentmodes", userId, cartId)];
}

- (NSString*)userPaymentModeUrlForUserId:(NSString*)userId andCartId:(NSString*) cartId {
    return [self.restPrefix stringByAppendingString:FORMAT(@"/users/%@/carts/%@/paymentmode", userId, cartId)];
}

- (NSString*)userPaypalExecuteUrlForUserId:(NSString*)userId andOrderId: (NSString*)orderId{
    return [self.restPrefix stringByAppendingString:FORMAT(@"/users/%@/orders/%@/paypal/execute", userId, orderId)];
}

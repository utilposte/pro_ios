...
- (NSString *)localizedMsgFromCartModification:(HYBCartModification*)cartModification {
    ...
    if ([statusCode isEqualToString:SUCCESS_TAG]) {
        if ([statusMsg hyb_isNotBlank]) {
            ...
            if ([quantityAdded intValue] == 1) {
                localizedMsg = [NSString stringWithFormat:@"%d produit ajouté", [quantityAdded intValue]];
            } else {
                localizedMsg = [NSString stringWithFormat:@"%d produits ajoutés", [quantityAdded intValue]];
            }
        }
    } else {
        if (statusMsg) {
            if ([quantityAdded intValue] == 1) {
                localizedMsg = [NSString stringWithFormat:@"%d produit ajouté", statusMsg, [quantityAdded intValue]];
            } else {
                localizedMsg = [NSString stringWithFormat:@"%d produits ajout(s", statusMsg, [quantityAdded intValue]];
            }
        } else {
            DDLogError(@"Problems while retrieving the message for the add to cart action: The status message is missing.");
            localizedMsg = [NSString stringWithFormat:@"Une erreur est survenue lors de l'ajout du produit au panier"];
        }
    }
    return localizedMsg;
}
    ...
    [self.restEngine POST:self.tokenURL
               withParams:params
                  ...
                  failure:^(AFHTTPRequestOperation *task, NSError *error) {
                      ...
                      block(NSLocalizedString(@"make_trusted_client_failed", nil), error);
                  }];
...
// Added by Rafik ISSOLAH - Bnum on 08/03/2016
//BEGIN
-(void)setUserInfoWithCCUToken:(NSDictionary *)CCUToken
                      username:(NSString *)username{
    
    [self storeSession:CCUToken forUsername:[username lowercaseString]];
    self.userId = [username lowercaseString];
    [self applyAuthToken:CCUToken[@"access_token"]];
    ...
    [self.restEngine GET:self.productsSearchURL
              withParams:params
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                     ...
                     NSString *productsGategory = [responseObject valueForKey:@"pagination.sorts"];
                     ...
                     //facets for filtre
                     NSArray *facetsObjectsFromWebServices = (NSArray*)[responseObject objectForKey:@"facets"];
                     NSMutableArray *tmpFacetsArray = nil;
                     
                     if([facetsObjectsFromWebServices hyb_isNotBlank]) {
                         tmpFacetsArray = [NSMutableArray array];
                         for (id object in facetsObjectsFromWebServices) {
                             HYBFacet *facet = [HYBFacet facetWithParams:(NSDictionary*)object];
                             [tmpFacetsArray addObject:facet];
                         }
                     }
                     
                     //sorts for tri
                     NSArray *sortObjectsFromWebServices = (NSArray*)[responseObject objectForKey:@"sorts"];
                     NSMutableArray *tmpSortsArray = nil;
                     
                     if([sortObjectsFromWebServices hyb_isNotBlank]) {
                         tmpSortsArray = [NSMutableArray array];
                         for (id object in sortObjectsFromWebServices) {
                             HYBSort *sort = [HYBSort sortWithParams:(NSDictionary*)object];
                             [tmpSortsArray addObject:sort];
                         }
                     }
                     ...
                     if([totalResults hyb_isNotBlank]) [tmpResults setValue:totalResults forKey:@"totalResults"];
                     if([tmpFacetsArray hyb_isNotBlank])[tmpResults setValue:tmpFacetsArray forKey:@"facets"];
                     if([tmpSortsArray hyb_isNotBlank])[tmpResults setValue:tmpSortsArray forKey:@"sorts"];
...
// *@brief
// */
//- (void)getProductsWithParamsAndDetails:(NSDictionary*)params
//                   andExecute:(void (^)(id, NSError *,NSInteger *,NSArray *))block {
//
//    NSLog(@"get Products Search %@", self.productsSearchURL);
//
//    @weakify(self);
...
//                     if([spellingSuggestion hyb_isNotBlank]) [tmpResults setValue:spellingSuggestion forKey:@"spellingSuggestion"];
//
//                     block([NSDictionary dictionaryWithDictionary:tmpResults], nil,totalResults.intValue,facetsObjectsFromWebServices);
//                 }
//
//                 failure:^(AFHTTPRequestOperation *operation, NSError *error) {
//                     //error anchor
...
//needs ROLE_CLIENT, ROLE_TRUSTED_CLIENT, ROLE_CUSTOMERMANAGERGROUP
...
- (void) getCurrentCartForUserId:(NSString*)userId
                      withParams:(NSDictionary*)params
                      andExecute:(void (^)(id, NSError *))block {
    
    NSString *url = [self userCartsURLForUserId:userId];
    
    if (![userId isEqualToString:GUEST_USER]) {
        url = [self userCurrentCartURLForUserId:userId];
    }
    
    NSLog(@"get Carts For User Id %@", url);
    
    [self.restEngine GET:url
              withParams:[NSDictionary FULLFields:params]
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                     HYBCart *cart = [HYBCart cartWithParams:(NSDictionary*)responseObject];
...
- (void)createCartDeliveryAddressForUserId:(NSString*)userId
    ...
    NSMutableDictionary *escapedParams = nil;
    NSString *str = [self userCartDeliveryAddressURLForUserId:userId andCartId:cartId];
    if (params != nil){
        str = [str stringByAppendingString: @"?"];
    }
    NSUInteger keyCount = [params count];
    NSUInteger num = 1;
    ...
    for(id key in params){
        ...
        [escapedParams setObject:value forKey:key];
        num ++;
    }

    
    [self.restEngine POST: str
               withParams: nil
                  success:^(AFHTTPRequestOperation *operation, id responseObject) {
                      HYBAddress *address = nil;
                      if([responseObject hyb_isNotBlank]) {
...
- (void)createCartPaymentdetailsForUserId:(NSString*)userId
    ...
    NSMutableDictionary *escapedParams = nil;
    NSString *str = [self userCartPaymentdetailsURLForUserId:userId andCartId:cartId];
    if (params != nil){
        str = [str stringByAppendingString: @"?"];
    }
    NSUInteger keyCount = [params count];
...
- (void)setCartVoucherForUserId:(NSString*)userId
    ...
    [self.restEngine POST:[self userCartVouchersURLForUserId:userId andCartId:cartId]
               withParams:params
                  success:^(AFHTTPRequestOperation *operation, id responseObject) {
                      block(SUCCESS_TAG,nil);
                  }
...
- (void)getOrdersForUserId:(NSString*)userId
    ...
    if (params) {
        params = [NSDictionary FULLFields:params];
    }
    ...
    [self.restEngine GET:[self userOrdersURLForUserId:userId]
              withParams:params
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                     ...
                     NSNumber *totalResults =  ((NSDictionary*)[responseObject objectForKey:@"pagination"])[@"totalResults"];
                     self.totalSearchResults = totalResults.intValue;
...
- (void)renewOrderForUserId:(NSString *)userId
                   withCode:(NSString *)code
                 andExecute:(void (^)(id, NSError *))block {
    
    NSLog(@"place Order For User Id %@",[self userOrdersURLForUserId:userId]);
    
    [self.restEngine GET:[self renewOrderForUserId:self.userId code:code]
              withParams:nil
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                     
                     NSDictionary *dic = responseObject;
                     NSInteger quantityAdded = 0;
                     NSInteger quantityRequested = 0;
                     
                     for (NSDictionary *entry in dic[@"cartModifications"]) {
                         NSLog(@"entry status %@", entry[@"statusCode"]);
                         quantityAdded = [entry[@"quantityAdded"] integerValue];
                         quantityRequested = [entry[@"quantity"] integerValue];
                     }
                     
                     if (quantityAdded == quantityRequested) {
                         block(@"success",nil);
...
- (void)getOrderWithNoParamsForUserId:(NSString*)userId
                         andOrderCode:(NSString*)code
                           andExecute:(void (^)(id, NSError *))block {
...
- (void)getOrderInvoiceForUserId:(NSString*)orderCode
                      andExecute:(void (^)(NSString *, NSError *))block  {
    
    NSLog(@"get Order Invoice For User Id and Order Code %@", orderCode);
    
    [self.restEngine GET:[self userOrderInvoiceURLForUserId:self.userId andOrderCode:orderCode]
              withParams:[NSDictionary FULLFields]
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                     NSString *invoiceUrl;
                     invoiceUrl = [responseObject objectForKey:@"invoiceURL"];
                     block(invoiceUrl,nil);
                 }
...
#pragma mark Payment Paypal

- (void)getPaymentModes:(NSString*)cartId
                      andExecute:(void (^)(BOOL, NSError *))block {
    
    NSLog(@"get payment modes For User Id and Cart Id %@",[self userPaymentModesUrlForUserId:self.userId andCartId:cartId]);
    
    [self.restEngine GET:[self userPaymentModesUrlForUserId:self.userId andCartId:cartId]
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                     NSLog(@"################# %@ ##############", responseObject);
                     NSDictionary *dic = responseObject;
                     NSDictionary *paymentModes = dic[@"paymentModes"];
                     for (NSDictionary *item in paymentModes) {
                         NSString *value = [item objectForKey:@"code"];
                         
                         if ([value isEqualToString:@"PAYPAL"]) {
                             // contain PAYPAL
                             block(YES, nil);
                             return;
                         }
                     }
                     // don't contain PAYPAL
                     block(NO, nil);
                 }
...
- (void)setPaymentMode:(NSString*)cartId
                     withParams:(NSDictionary*)params
                     andExecute:(void (^)(id, NSError *))block {
    
    NSLog(@"set payment mode For User Id and Cart Id %@",[self userPaymentModeUrlForUserId:self.userId andCartId:cartId]);
    
    [self.restEngine PUT:[self userPaymentModeUrlForUserId:self.userId andCartId:cartId] withParams:params
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)executePayPalPayment:(NSString*)orderId
                           withParams:(NSDictionary*)params
                           andExecute:(void (^)(id, NSError *))block {
    
    NSLog(@"execute PayPal payment For User Id and Order Id %@",[self userPaypalExecuteUrlForUserId:self.userId andOrderId:orderId]);
    
    [self.restEngine POST:[self userPaypalExecuteUrlForUserId:self.userId andOrderId:orderId] withParams:params
                  success:^(AFHTTPRequestOperation *operation, id responseObject) {
                      
                      if ([[responseObject objectForKey:@"paymentStatus"] isEqualToString:@"approved"]) {
                          block(SUCCESS_TAG, nil);
                      } else {
                          block(nil, nil);
                      }

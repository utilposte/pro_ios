    ...
    if (!_baseURL) {
       ...
       if (self.useSSL) protocol   = @"https://";
        ...
        if([port hyb_isNotBlank]){
            result = FORMAT(@"%@%@:%@/eboutiquecommercewebservices", protocol, host, port);
        } else {
            result = FORMAT(@"%@%@/eboutiquecommercewebservices", protocol, host);
        }
...
- (NSString *)baseURLNoRest {
    return [self.baseURL stringByDeletingLastPathComponent];
}
